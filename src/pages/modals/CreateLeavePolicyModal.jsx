// src/components/modals/CreateLeavePolicyModal.jsx
...
const CreateLeavePolicyModal = ({ isOpen, onClose, onPolicyCreated }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.maxLeavesPerYear || formData.maxLeavesPerYear <= 0) {
      toast.warning("Max leaves per year must be a positive number");
    ...
    try {
      ...
      const { data } = await leavePolicyAPI.create(payload);
      onPolicyCreated();
      ...
      setError(err.response?.data?.error || err.message || "Error creating leave policy");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      ...
      >
        ...
        <div className="flex justify-between items-center p-6 border-b" style={{ borderColor: themeColors.border }}>
          <h2 className="text-xl font-semibold">Create New Leave Policy</h2>
          <button
            onClick={handleClose}
